...
from utils.utils import eval_mo, linearly_decaying_epsilon
from rl_algorithm import RLAlgorithm
from gpi import GPI
from utils.buffer import ReplayBuffer
from utils.prioritized_buffer import PrioritizedReplayBuffer
import wandb as wb 
...
class SF(RLAlgorithm):
    ...
    def train(self, w: np.array):
        ...
        if self.log and self.num_timesteps % 1000 == 0:
            self.nl.log_metric(metric_value=self.epsilon, metric_name='epsilon', step=self.num_timesteps, mode='metrics')
            self.nl.log_metric(metric_value=np.dot(td_error, w), metric_name='td_error', step=self.num_timesteps, mode='metrics')
            if self.per:
                self.nl.log_metric(metric_value=new_priorities.mean(), metric_name='mean_priority', step=self.num_timesteps,mode='metrics')
    ...
    def learn(self, total_timesteps, total_episodes=None, reset_num_timesteps=True, eval_env=None, eval_freq=1000, w=np.array([1.0,0.0])):
        ...
        for _ in range(1, total_timesteps+1):
            ...
            if eval_env is not None and self.log and self.num_timesteps % eval_freq == 0:
                self.gpi.get_metric(self.num_timesteps)
                total_reward, discounted_return, total_vec_r, total_vec_return, _ = eval_mo(self, eval_env, w)
                self.nl.log_metric(metric_value=total_reward, metric_name='eval_total_reward', step=self.num_timesteps,mode='metrics')
                self.nl.log_metric(metric_value=discounted_return, metric_name='eval_discounted_return', step=self.num_timesteps,mode='metrics')
                for i in range(episode_vec_reward.shape[0]):
                    self.nl.log_metric(metric_value=total_vec_r[i], metric_name=f'eval_total_reward_obj{i}', step=self.num_timesteps,mode='metrics')
                    self.nl.log_metric(metric_value=total_vec_return[i], metric_name=f'eval_discounted_return_obj{i}', step=self.num_timesteps,mode='metrics')
            ...
            if done:
                ...
                if num_episodes % 1000 == 0:
                    #self.gpi.get_metric(num_episodes)
                    print(f"Episode: {self.num_episodes} Step: {self.num_timesteps}, Ep. Total Reward: {episode_reward}, {episode_vec_reward}")
                if self.log:
                    #self.gpi.get_metric(num_episodes)
                    self.nl.log_metric(metric_value=episode_reward, metric_name='episode_reward', step=self.num_timesteps,mode='metrics')
                    self.nl.log_metric(metric_value=self.num_episodes, metric_name='episode', step=self.num_timesteps,mode='metrics')
                    for i in range(episode_vec_reward.shape[0]):
                        self.nl.log_metric(metric_value=episode_vec_reward[i], metric_name=f'episode_reward_obj{i}', step=self.num_timesteps,mode='metrics')

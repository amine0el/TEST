...
from utils.utils import random_weights, hypervolume, policy_evaluation_mo
...
if __name__ == "__main__":
    ...
    def solve(w):
        return np.array(list(map(float, input(w).split())), dtype=np.float32)

    m = 2
    ols = OLS(m=m, epsilon=0.01) #, min_value=0.0, max_value=1 / (1 - 0.95) * 1)
    while not ols.ended():
        w = ols.next_w()
        print("w:", w)
        value = solve(w)
